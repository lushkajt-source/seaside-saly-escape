...
import { Loader2, LogOut, Search, CalendarDays, Users, CheckCircle, XCircle, Clock, Plus, Trash2, UserPlus } from "lucide-react";
...
const AdminDashboard = () => {
  const { user, isAdmin, loading: authLoading, signOut } = useAuth(true, "admin");
  ...
  const updateStatus = async (id: string, newStatus: string, reason?: string) => {
    ...
    if (error) {
      ...
      if (updatedBooking) {
        ...
        else if (newStatus === "declined") sendEmail("declined", bookingWithUpdate);
  ...
  const handleCreateEmployee = async () => {
    if (!newEmpEmail || !newEmpPassword || !newEmpName) {
    ...
    try {
      const { data, error } = await supabase.functions.invoke("manage-employees", {
        body: { action: "create", email: newEmpEmail, password: newEmpPassword, name: newEmpName },
      });
      if (error) throw error;
  ...
  const handleDeleteEmployee = async (empId: string) => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke("manage-employees", {
        body: { action: "delete", userId: empId },
      });
      if (error) throw error;
      if (data?.error) throw new Error(data.error);
    ...
    <div className="min-h-screen" style={{ background: "hsl(30 10% 12%)" }}>
      ...
      <div className="max-w-7xl mx-auto p-6 space-y-6">
        <Tabs defaultValue="bookings">
          <TabsList className="border-none" style={{ background: "hsl(30 10% 18%)" }}>
            <TabsTrigger value="bookings" className="data-[state=active]:text-white" style={{ color: "hsl(30 10% 55%)" }}>Bookings</TabsTrigger>
            <TabsTrigger value="employees" className="data-[state=active]:text-white" style={{ color: "hsl(30 10% 55%)" }}>Employees</TabsTrigger>
          </TabsList>
                        ...
                        <TableRow key={b.id} style={{ background: "hsl(30 10% 14%)", borderColor: "hsl(30 10% 20%)" }} className="transition-colors hover:!bg-[hsl(30,10%,17%)]">
                          ...
                          <TableCell>
                            ...
                            {b.status === "declined" && b.decline_reason && (
                              <p className="text-[10px] mt-1 truncate max-w-[100px]" style={{ color: "hsl(0 70% 55%)" }} title={b.decline_reason}>{b.decline_reason}</p>
                            )}
                          </TableCell>
                          <TableCell>
                            <div className="flex gap-2">
                              {b.status === "pending" && (
                                <>
                                  ...
                                  <Button size="sm" variant="ghost" disabled={updatingId === b.id} onClick={() => { setDeclineBooking(b); setDeclineReason(""); setDeclineDialogOpen(true); }} className="text-xs" style={{ color: "hsl(0 70% 55%)", background: "hsl(0 70% 50% / 0.1)" }}>
                                    Decline
                                  </Button>
                                </>
                              )}
          ...
          <TabsContent value="employees" className="space-y-6 mt-6">
            <div className="flex justify-between items-center">
              <h2 className="text-xl font-serif" style={{ fontFamily: "'Cormorant Garamond', serif", ...goldText }}>Employee Accounts</h2>
              <Button onClick={() => setNewEmpOpen(true)} className="text-white border-none" style={{ background: "linear-gradient(135deg, hsl(38 55% 55%), hsl(38 45% 40%))" }}>
                <UserPlus className="w-4 h-4 mr-2" /> Add Employee
              </Button>
            </div>

            {empLoading ? (
              <div className="flex justify-center py-20"><Loader2 className="w-8 h-8 animate-spin" style={goldText} /></div>
            ) : employees.length === 0 ? (
              <div className="text-center py-20" style={mutedText}>
                <Users className="w-12 h-12 mx-auto mb-3 opacity-40" />
              ...
              <div className="rounded-lg overflow-hidden" style={{ border: "1px solid hsl(30 10% 22%)" }}>
                <Table>
                  <TableHeader>
                    <TableRow style={{ background: "hsl(30 10% 16%)", borderColor: "hsl(30 10% 22%)" }}>
                      <TableHead style={{ ...goldText, fontWeight: 600, fontSize: "0.7rem", textTransform: "uppercase" }}>Email</TableHead>
                      <TableHead style={{ ...goldText, fontWeight: 600, fontSize: "0.7rem", textTransform: "uppercase" }}>Created</TableHead>
                      <TableHead style={{ ...goldText, fontWeight: 600, fontSize: "0.7rem", textTransform: "uppercase" }}>Actions</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {employees.map((emp) => (
                      <TableRow key={emp.id} style={{ background: "hsl(30 10% 14%)", borderColor: "hsl(30 10% 20%)" }}>
                        <TableCell style={lightText}>{emp.email}</TableCell>
                        <TableCell style={mutedText} className="text-xs">{emp.created_at ? format(new Date(emp.created_at), "MMM d, yyyy") : "—"}</TableCell>
                        <TableCell>
                          <Button size="sm" variant="ghost" disabled={deletingEmpId === emp.id} onClick={() => handleDeleteEmployee(emp.id)} style={{ color: "hsl(0 70% 55%)" }}>
                            {deletingEmpId === emp.id ? <Loader2 className="w-3 h-3 animate-spin" /> : <Trash2 className="w-4 h-4" />}
                          </Button>
                        </TableCell>
                      </TableRow>
                    ))}
      ...
      <Dialog open={declineDialogOpen} onOpenChange={setDeclineDialogOpen}>
        <DialogContent style={{ background: "hsl(30 10% 18%)", border: "1px solid hsl(30 10% 25%)", color: "hsl(40 30% 85%)" }}>
          <DialogHeader>
            ...
            <DialogDescription style={mutedText}>Provide a reason for declining {declineBooking?.guest_name}'s reservation.</DialogDescription>
          </DialogHeader>
          <Textarea value={declineReason} onChange={(e) => setDeclineReason(e.target.value)} placeholder="Enter the reason..." className="border-none min-h-[100px]" style={inputStyle} />
          <DialogFooter>
            <Button variant="ghost" onClick={() => setDeclineDialogOpen(false)} style={{ color: "hsl(30 10% 55%)" }}>Cancel</Button>
            <Button onClick={handleDecline} disabled={!declineReason.trim() || updatingId === declineBooking?.id} style={{ background: "hsl(0 70% 45%)", color: "white", border: "none" }}>
      ...
      <Dialog open={cancelDialogOpen} onOpenChange={setCancelDialogOpen}>
        <DialogContent style={{ background: "hsl(30 10% 18%)", border: "1px solid hsl(30 10% 25%)", color: "hsl(40 30% 85%)" }}>
          <DialogHeader>
            ...
            <DialogDescription style={mutedText}>Provide a reason for cancelling {cancelBooking?.guest_name}'s reservation.</DialogDescription>
          </DialogHeader>
        ...
        <DialogContent style={{ background: "hsl(30 10% 18%)", border: "1px solid hsl(30 10% 25%)", color: "hsl(40 30% 85%)" }} className="max-w-lg">
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div><Label style={goldText}>Email</Label><Input type="email" value={newBooking.guest_email} onChange={(e) => setNewBooking(p => ({ ...p, guest_email: e.target.value }))} className="border-none" style={inputStyle} /></div>
              <div><Label style={goldText}>Phone</Label><Input value={newBooking.phone} onChange={(e) => setNewBooking(p => ({ ...p, phone: e.target.value }))} className="border-none" style={inputStyle} /></div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div><Label style={goldText}>Check-in *</Label><Input type="date" value={newBooking.check_in} onChange={(e) => setNewBooking(p => ({ ...p, check_in: e.target.value }))} className="border-none" style={inputStyle} /></div>
              <div><Label style={goldText}>Check-out *</Label><Input type="date" value={newBooking.check_out} onChange={(e) => setNewBooking(p => ({ ...p, check_out: e.target.value }))} className="border-none" style={inputStyle} /></div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
              ...
              <div><Label style={goldText}>Guests</Label><Input type="number" min="1" value={newBooking.guests_count} onChange={(e) => setNewBooking(p => ({ ...p, guests_count: e.target.value }))} className="border-none" style={inputStyle} /></div>
            </div>
            <div><Label style={goldText}>Special Request</Label><Textarea value={newBooking.special_request} onChange={(e) => setNewBooking(p => ({ ...p, special_request: e.target.value }))} className="border-none" style={inputStyle} /></div>
          </div>
          <DialogFooter>
      ...
      <Dialog open={newEmpOpen} onOpenChange={setNewEmpOpen}>
        <DialogContent style={{ background: "hsl(30 10% 18%)", border: "1px solid hsl(30 10% 25%)", color: "hsl(40 30% 85%)" }}>
          <DialogHeader>
            <DialogTitle style={goldText}>Create Employee Account</DialogTitle>
            <DialogDescription style={mutedText}>The employee will use these credentials to log in.</DialogDescription>
          </DialogHeader>
          <div className="space-y-3">
            <div><Label style={goldText}>Name</Label><Input value={newEmpName} onChange={(e) => setNewEmpName(e.target.value)} className="border-none" style={inputStyle} /></div>
            <div><Label style={goldText}>Email</Label><Input type="email" value={newEmpEmail} onChange={(e) => setNewEmpEmail(e.target.value)} className="border-none" style={inputStyle} /></div>
            <div><Label style={goldText}>Password</Label><Input type="password" value={newEmpPassword} onChange={(e) => setNewEmpPassword(e.target.value)} className="border-none" style={inputStyle} /></div>
          </div>
          <DialogFooter>
            <Button variant="ghost" onClick={() => setNewEmpOpen(false)} style={{ color: "hsl(30 10% 55%)" }}>Cancel</Button>
            <Button onClick={handleCreateEmployee} disabled={creatingEmp} className="text-white border-none" style={{ background: "linear-gradient(135deg, hsl(38 55% 55%), hsl(38 45% 40%))" }}>
              {creatingEmp ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : null}
              Create Account
            </Button>
          </DialogFooter>
        </DialogContent>
